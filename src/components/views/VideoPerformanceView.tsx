...
interface VideoPerformanceViewProps {
    stats?: any;
    analyticsData?: any;
}

export default function VideoPerformanceView({ stats, analyticsData }: VideoPerformanceViewProps) {
    const totalViews = analyticsData?.videos?.reduce((sum: number, v: any) => sum + v.views, 0).toLocaleString() || '---';
    const brandSubs = stats?.subscriber_count?.toLocaleString() || '---';
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <StatCard title="Total Views" value={totalViews} change="+8%" changeLabel="vs Previous" icon={<Eye className="w-5 h-5 text-green-600" />} iconBg="bg-green-50" />
                <ApiGapOverlay>
                ...
                <StatCard title="Brand Subs" value={brandSubs} change="+320" changeLabel="Conversion" icon={<Users className="w-5 h-5 text-orange-600" />} iconBg="bg-orange-50" />
            </div>
